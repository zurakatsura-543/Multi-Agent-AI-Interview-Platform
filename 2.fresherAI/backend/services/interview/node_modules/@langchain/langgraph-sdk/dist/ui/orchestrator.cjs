...
var StreamOrchestrator = class {
	...
	*/
	get interrupts() {
		const v = this.values;
		if (v != null && "__interrupt__" in v && Array.isArray(v.__interrupt__)) return require_interrupts.userFacingInterruptsFromValuesArray(v.__interrupt__);
		if (this.isLoading) return [];
		const taskInterrupts = require_interrupts.userFacingInterruptsFromThreadTasks((this.branchContext.threadHead?.tasks ?? []).flatMap((t) => t.interrupts ?? []));
	...
	*/
	...
	#flushPendingHeadlessToolInterrupts(values) {
		require_headless_tools.flushPendingHeadlessToolInterrupts(values, this.#options.tools, this.#handledHeadlessToolInterruptIds, {

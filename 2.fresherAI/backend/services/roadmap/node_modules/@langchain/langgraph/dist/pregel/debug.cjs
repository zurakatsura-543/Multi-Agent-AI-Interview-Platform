...
function buildTaskMetadata(config) {
	...
	if (config.metadata != null) {
		for (const [key, value] of Object.entries(config.metadata)) if (!_langchain_langgraph_checkpoint.EXCLUDED_METADATA_KEYS.has(key)) metadata[key] = value;
	}
	const filteredTags = require_config.filterToUserTags(config.tags);
